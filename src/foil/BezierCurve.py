...
class Bezier:
    ...
    def on_motion(self, event):
        ...
        if self.index_02 is None:
            return
        self.xs[self.index_02] = (
            event.xdata
        )  # The coordinates of the mouse override the coordinates of the selected point
    ...
    def on_release(self, event):
        ...
        if (
            self.pick == 1 and self.motion != 1
        ):  # If it is a selected point, but not a drag point, then reduce the order
            x = self.xs
            xdata = event.xdata
            ydata = event.ydata
            index_01 = 0
...
fig = plt.figure(
    2, figsize=(12, 6)
)  # Create the second drawing object, 1200*600 pixels
